...
async function updateParentText(projectId, itemId, fieldId, text) {
  const mutation = `
  ...
  `;
  const variables = { input: { projectId, itemId, fieldId, value: { text } } };
...
(async ()=>{
  try{
    ...
    const textValue = `Depends on project item ${parentItemId} (issue #${PARENT_ISSUE})`;
    console.log(`Writing text to Parent issue field (${parentField.id}) on item ${childItemId}: ${textValue}`);
    const resp = await updateParentText(projectId, childItemId, parentField.id, textValue);
