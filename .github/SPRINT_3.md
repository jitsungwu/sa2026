...
## Issue #38: 學生：舉手後可看到舉手順序（enhancement）
- State: OPEN
- URL: https://github.com/jitsungwu/sa2026/issues/38

說明
- 身為 學生，
  我想要 舉手後可以看到舉手順序，
  因此我可以 了解目前誰已經舉手、我的位置在哪裡。
...
- Scenario：舉手並查看順序
  ...
  - Then (預期結果)：
    - 教師可看到班級的舉手狀況
    - **AND 學生也可以看到舉手順序** ← 本 Issue 重點
  - 預期行為：系統應即時顯示舉手的順序清單，包括各組舉手時間/順序

相依註記
- Issue #38 改善 Issue #17（舉手功能）
- 與 Issue #36、#37 的整合：當設定報告組與優先發問組後，舉手順序的顯示邏輯需對應調整

需澄清問題
- 顯示順序的排序規則：依舉手時間順序？或按組號？
- 是否需在學生儀表板上顯示「我目前的舉手位置」（例如 3/8）？
- 舉手被 resolve 或被跳過後，是否從順序清單中移除？
- 在報告/優先發問模式下，舉手順序是否仍然顯示？或隱藏？
- 視覺設計：順序清單要以卡片、列表或標籤形式呈現？是否遵循 `.github/SCAFFOLD_UI_SPEC.md`？
- 資料來源：是否需透過 `hands_raised` 集合查詢並排序？或另建 `hands_order` 輔助欄位？
- 實時更新：是否使用 `onSnapshot` 即時監聽舉手順序變化？

---

_檔案由 Copilot 自動更新；請確認 Issue 內容後進行開發。_
